document.addEventListener("DOMContentLoaded", () => {
  ...
  const receiverGithubDisplay = document.getElementById('receiverGithubDisplay');
    ...
    (data) => {
      ...
      if (data.github_owner && data.github_repo) {
        githubDiv.innerHTML = `
          ...
          <strong>Link:</strong> <a href="${data.github_url}" target="_blank" class="underline text-green-400">${data.github_url}</a>
        `;
  ...
  templateSelect.addEventListener("change", () => {
    ...
    if (selected && selected.imageUrl) {
      templatePreview.innerHTML = `
        <img src="${selected.imageUrl}" alt="${selected.templateName}" class="rounded shadow mb-2 max-h-20 border border-gray-700 mx-auto" />
        <div class="text-xs text-gray-400 text-center">${selected.description || ""}</div>
      `;
    } else {
      templatePreview.innerHTML = "";
    }
